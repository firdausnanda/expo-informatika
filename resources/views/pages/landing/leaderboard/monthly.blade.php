...
@section('content')
    <div class="container py-5">
        ...
        <div class="filter-tabs" role="tablist" aria-label="Filter options">
            ...
            <button type="button" class="active" aria-pressed="true">Monthly</button>
        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#month').select2({
                ...
                theme: "bootstrap-5",
                ajax: {
                    url: "{{ route('tahun-akademik') }}",
                    type: "GET",
                    dataType: "JSON",
                    delay: 250,
                    processResults: function(data) {
                        const items = data.data || data;
                        const results = items.tahunAkademik.map(function(item) {
                            return {
                                id: item.id,
            ...
            @if (isset($tahunAkademikSelected))
                var selectedId = "{{ $tahunAkademikSelected->id }}";
                var select = $('#month');

                // Fetch data terpilih via AJAX jika perlu
                $.ajax({
                    url: "{{ route('tahun-akademik') }}",
                    type: "GET",
                    dataType: "json"
                    ...
                    const items = data.data || data;
                    const selectedItem = items.tahunAkademik.find(item => item.id == selectedId);

                    if (selectedItem) {
                        // Buat option baru dan pilih
                        var option = new Option(
                            selectedItem.tahun_akademik + ' ' + selectedItem.semester,
                            selectedItem.id,
                            true,
                            true
                        );
            ...
            $('#month').on('change', function() {
                ...
                table.ajax.reload();
                loadTopCards(selectedId);

            });

            // Load Top Cards
            function loadTopCards(monthId) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.meta.code === 200) { // Sesuaikan dengan kode success ResponseFormatter
                            ...
                            $('.total-projects').text(response.data.count);
